...
function calcolaIndiceTarget(offset, tabWidth, activeIdx, count) {
  ...
  return Math.max(0, Math.min(count - 1, Math.floor(center / tabWidth)));
}
...
function MobileTopBar() {
  ...
  useEffect(() => {
    if (!aperto) return;
    const close = (e) => { if (!menuRef.current?.contains(e.target)) setAperto(false); };
  ...
  const doLogin = () => { hapticLight(); window.location.href = getTwitchLoginUrl(location.pathname); };
  const doLogout = () => { hapticLight(); logout(); setAperto(false); };
  const go = (path) => { hapticLight(); setAperto(false); navigate(path); };
  ...
  return (
    <div className="mobile-topbar" ref={menuRef}>
      ...
      <button
        ...
        onClick={() => { hapticLight(); if (!isLoggedIn) doLogin(); else setAperto(v => !v); }}
      >
        {isLoggedIn ? <img src={twitchAvatar || LOGO_AVATAR_FALLBACK} alt="" /> : <LogIn size={20} />}
      </button>
      <AnimatePresence>
        {aperto && isLoggedIn && (
          <motion.div className="mobile-profile-menu glass-panel" initial={{ opacity: 0, y: -8, scale: 0.96 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -8, scale: 0.96 }} transition={{ type: 'spring', stiffness: 360, damping: 28 }}>
            <div className="mobile-profile-menu-head">
              <img src={twitchAvatar || LOGO_AVATAR_FALLBACK} alt="" />
              <div><strong>{twitchDisplay || twitchUser}</strong><span>@{twitchUser}</span></div>
            </div>
            <button type="button" onClick={() => go(`/profilo/${twitchUser}`)}><UserRound size={17} /> {t('settings.vedi_profilo')}</button>
            <button type="button" onClick={() => { cicla(); hapticLight(); }}><TemaIcon size={17} /> {labelTema}</button>
            <button type="button" onClick={() => go('/impostazioni')}><SettingsIcon size={17} /> {t('nav.impostazioni')}</button>
            <button type="button" className="danger" onClick={doLogout}><LogOut size={17} /> {t('settings.esci')}</button>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
...
function MobileTabBar({ activePath, haNonLetti }) {
  ...
  useLayoutEffect(() => { const tw = getTabWidth(); if (tw > 0) pillX.jump(activeIdx * tw); }, []); // eslint-disable-line react-hooks/exhaustive-deps
  useEffect(() => { if (!isDraggingRef.current) { const tw = getTabWidth(); if (tw > 0) fmAnimate(pillX, activeIdx * tw, { type: 'spring', stiffness: 420, damping: 34, mass: 0.9 }); } }, [activeIdx, getTabWidth, pillX]);
  useEffect(() => {
    const handler = () => { if (!isDraggingRef.current) { const tw = getTabWidth(); if (tw > 0) pillX.jump(activeIdxRef.current * tw); } };
    window.addEventListener('resize', handler);
    return () => window.removeEventListener('resize', handler);
  }, [getTabWidth, pillX]);

  const handlePanStart = useCallback(() => { isDraggingRef.current = true; prevSnapRef.current = activeIdxRef.current; fmAnimate(pillScale, 1.04, { type: 'spring', stiffness: 400, damping: 22 }); }, [pillScale]);
  const handlePan = useCallback((_, info) => {
    if (!isDraggingRef.current) return;
    const tw = getTabWidth(); if (!tw) return;
    const ai = activeIdxRef.current;
    let targetX = ai * tw + info.offset.x;
    const minX = 0; const maxX = (count - 1) * tw;
    ...
    if (idx !== prevSnapRef.current) { hapticLight(); prevSnapRef.current = idx; }
  ...
  const handlePanEnd = useCallback((_, info) => {
    ...
    if (targetIdx !== ai) { hapticLight(); navigate(MOBILE_LINKS[targetIdx].path); }
    ...
    <nav className="mobile-tab-bar" aria-label={t('nav.aria.main')}>
      <motion.div ref={itemsRef} className="mobile-tab-items" onPanStart={handlePanStart} onPan={handlePan} onPanEnd={handlePanEnd} style={{ touchAction: 'pan-y' }}>
        ...
        {MOBILE_LINKS.map(({ path, labelKey, Icon }) => {
          ...
          return <Link key={path} to={path} className={`tab-item${isActive ? ' active' : ''}`} aria-label={label} aria-current={isActive ? 'page' : undefined} onClick={() => hapticLight()} onTouchStart={() => prefetchPagina(path)} onPointerEnter={() => prefetchPagina(path)}><motion.span className="tab-icon" animate={{ y: isActive ? -2 : 0, scale: isActive ? 1.12 : 1 }} transition={{ type: 'spring', stiffness: 420, damping: 26 }}><span style={{ position: 'relative', display: 'flex' }}><Icon size={22} />{path === '/chat' && haNonLetti && <span className="tab-pallino" aria-hidden="true" />}</span></motion.span><motion.span className="tab-label" animate={{ opacity: isActive ? 1 : 0.5 }} transition={{ duration: 0.2 }}>{label}</motion.span></Link>;
        })}
      </motion.div>
    </nav>
  );
...
export default function Navbar() {
  ...
  return <><MobileTopBar /><AnimatePresence>{headerVisible && <motion.nav className="navbar-container glass-panel" initial={{ y: -80, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: -80, opacity: 0 }} transition={{ type: 'spring', stiffness: 340, damping: 32, mass: 0.8 }}><div className="navbar-content"><Link to="/" className="navbar-logo" aria-label={t('nav.aria.logo')}><img src={LOGO_URL} alt="ANDRYXify" className="navbar-logo-img" width="220" height="44" fetchPriority="high" /></Link><div ref={linksContainerRef} className="nav-links" style={{ position: 'relative', overflow: 'visible' }} onMouseLeave={() => setHoveredPath(null)}>{pillReady && <motion.div className="liquid-nav-pill" initial={false} animate={{ left: pillPos.left, width: pillPos.width }} transition={{ type: 'spring', stiffness: 420, damping: 34, mass: 0.8 }} />}{NAV_LINKS.map(({ path, labelKey, Icon }) => { const isActive = location.pathname === path; const label = t(labelKey); return <div key={path} ref={el => { linkRefs.current[path] = el; }} style={{ position: 'relative', zIndex: 1 }} onMouseEnter={() => { setHoveredPath(path); prefetchPagina(path); }} onFocus={() => prefetchPagina(path)}><Link to={path} className={`nav-link${isActive ? ' active' : ''}`}><motion.span whileHover={{ scale: 1.15, rotate: 4 }} style={{ display: 'flex', pointerEvents: 'none', position: 'relative' }}><Icon size={16} />{path === '/chat' && haNonLetti && <span className="nav-pallino" aria-hidden="true" />}</motion.span><span className="nav-label" style={{ pointerEvents: 'none' }}>{label}</span></Link></div>; })}</div><motion.button onClick={() => { cicla(); hapticLight(); }} className="nav-link nav-tema-toggle" aria-label={labelTema} title={labelTema} style={{ marginLeft: '0.25rem', border: 'none', cursor: 'pointer', background: 'transparent' }} whileHover={{ scale: 1.12 }} whileTap={{ scale: 0.9, rotate: 15 }} transition={{ type: 'spring', stiffness: 400, damping: 20 }}><motion.span key={modalita} initial={{ opacity: 0, rotate: -30, scale: 0.7 }} animate={{ opacity: 1, rotate: 0, scale: 1 }} transition={{ type: 'spring', stiffness: 380, damping: 22 }} style={{ display: 'flex' }}>{React.createElement(ICONE_TEMA[modalita], { size: 17 })}</motion.span></motion.button><Link to="/impostazioni" className="nav-link nav-settings" aria-label={t('nav.impostazioni')} style={{ marginLeft: '0.25rem', display: 'flex', alignItems: 'center' }}><SettingsIcon size={17} /></Link></div></motion.nav>}</AnimatePresence><MobileTabBar activePath={location.pathname} haNonLetti={haNonLetti} /></>;
}
