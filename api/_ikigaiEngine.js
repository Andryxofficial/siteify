...
function detectIntent(domandaAttuale, cronologia = [], contestoPagina = {}) {
  ...
  if (/\btag\b|\btags\b|hashtag|macro\s?categorie|macro-categories|smart categories|categor[ií]as inteligentes|tendenz|trend|tendencias/.test(attuale)) return 'tags';
  if (/classific|leaderboard|ranking|\bxp\b|livell|level|nivel|premi|premio|reward|premios|ranking|vip|campione|champion|campe[oó]n/.test(attuale)) return 'leaderboard';
  if (/notific|notification|push|vibraz|vibration|vibraci[oó]n|suon|sound|sonido|menzion|mention|menci[oó]n|ore silenziose|quiet hours|horas silenciosas|avvisi|alerts/.test(attuale)) return 'notifications';
  if (/privacy|privacidad|sicurezza|security|seguridad|solo amici|friends only|solo amigos|visibilit|visibility|visibilidad|privat|private|privado|dati|data|datos|chat private|private chats|chats privados/.test(attuale)) return 'privacy';
  if (/login|logout|twitch|account|profilo|profile|perfil|accesso|sign in|iniciar sesi[oó]n/.test(attuale)) return 'account';
  if (/giochi|gioco|gaming|giocare|games|game|play|juegos|jugar/.test(attuale)) return 'games';
  if (/socialify|post|feed|rispost|reply|respuest|thread|preferit|favorite|favorit|like|comment/.test(attuale)) return 'socialify';
...
function routeMatches(question, intent, orientamento = {}) {
  ...
  return ROUTES.map(route => {
    ...
    if (intent === 'privacy' && route.path === '/privacy') score += 5;
...
function rispostaDaMemorizzare(intent) {
  const map = {
    ...
    privacy: 'La privacy copre profilo, chat private, notifiche, Ikigai, dati adattivi e cancellazione.',
  };
...
export async function interpellaIkigai({ domanda, question, cronologia = [], history = [], contestoPagina = {}, pageContext = {}, req = null, corpo = {} } = {}) {
  ...
  if (redis) {
    ...
    redis.lpush('ikigai:domande:recenti', JSON.stringify({ q, intent, lingua, pagina: pagina?.pathname || '', ts: Date.now() })).catch(() => {});
...
export async function statoIkigai() {
  ...
  return { ok: true, name: 'Ikigai', engine: 'andryx-ikigai-natural-local-helper', externalApis: false, knowledgeSections: sections.map(s => s.title), stats, memoria };
}
