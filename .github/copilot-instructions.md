...
| 2026-04-22 | **🖼️ 7TV upload custom emote + auto-padder**: nuova action `seventv_upload` in `api/mod-emotes.js` (POST) — riceve `{ name, contentType, dataBase64, tags?, addToSet? }`, valida (regex nome `[A-Za-z0-9_-]{2,25}`, MIME PNG/JPEG/WebP/GIF/APNG/AVIF, max 6&nbsp;MB), costruisce multipart `FormData` con campi `name`, `tags`, `image`, fa POST a `https://7tv.io/v3/emotes` col token broadcaster persistito, recupera `emote_id` dalla risposta e (se `addToSet:true`, default) lo aggiunge al set del canale via mutation GraphQL `emoteSet.emotes(action: ADD)`. `bodyParser.sizeLimit` portato a `8mb` con `export const config`. Frontend `src/pages/mod/Emote.jsx`: nuovo pannello "Carica emote custom" nel tab 7TV (visibile se `tokenPresent`), con file input drag-friendly, anteprima 96×96 su sfondo a scacchi, rilevamento dimensioni + alpha + animata (sample 64×64 alpha-channel), checkbox "Pad a quadrato N×N (sfondo trasparente)" che usa un canvas per centrare l'immagine su tela `max(w,h)` quadrata trasparente (preserva alfa originale, no stretch); per immagini animate (GIF/APNG) skip padding e upload as-is. Nome emote validato live. Toast esiti. Lint + build verde (chunk Emote 21.7kB / 6.11kB gz). | `api/mod-emotes.js`, `src/pages/mod/Emote.jsx`, `.github/copilot-instructions.md` |
