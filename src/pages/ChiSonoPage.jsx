...
import {
  ...
  Twitch, MapPin, Cake, Heart, Star, Sparkles,
  ExternalLink, Shirt,
} from 'lucide-react';
import SEO from '../components/SEO';
import { useLingua } from '../contexts/LinguaContext';
import { chiSonoExtraText } from '../i18n/chiSonoExtra';
...
function MandyMashwearSection({ lingua }) {
    ...
    >
      <div
        ...
        style={{
          ...
          height: 260,
          borderRadius: '50%',
          background: 'radial-gradient(circle, rgba(224,64,251,.18), rgba(0,229,255,.08), transparent 68%)',
          filter: 'blur(18px)',
          pointerEvents: 'none',
        }}
      />
      <div
        style={{
          ...
          border: '1px solid color-mix(in srgb, var(--primary) 26%, var(--vetro-bordo-colore))',
          boxShadow: 'inset 0 1px 0 rgba(255,255,255,.28), 0 12px 28px rgba(224,64,251,.12)',
          position: 'relative',
          zIndex: 1,
        }}
      >
      ...
      <div style={{ position: 'relative', zIndex: 1, minWidth: 0 }}>
        ...
        >
          <Sparkles size={14} /> {txt.kicker}
        </span>
        <h2 style={{ margin: '0 0 .65rem', fontSize: '1.25rem', fontWeight: 900, fontFamily: "'Space Grotesk', 'Outfit', sans-serif" }}>
          {txt.title}
        </h2>
        <p style={{ margin: 0, color: 'var(--text-main)', lineHeight: 1.75 }}>
          {txt.body}
        </p>
        <p style={{ margin: '.8rem 0 0', color: 'var(--text-muted)', lineHeight: 1.65, fontSize: '.92rem' }}>
          {txt.note}
        </p>
        <a
          href={MANDY_URL}
  ...
  return (
    <div className="main-content">
      ...
      <section className="header hero-section" style={{ paddingTop: '2.5rem', paddingBottom: '0' }}>
        ...
        <motion.div {...su(0)} style={{ display: 'flex', justifyContent: 'center', marginBottom: '1.2rem' }}>
        ...
        <motion.h1 className="title" {...su(0.08)}>{t('chi-sono.titolo')}</motion.h1>
      ...
      <motion.div className="glass-panel" style={{ padding: '1.5rem' }} {...su(0.18)}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '1rem' }}>
          {FATTI.map(f => (
            <div key={f.label} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', gap: '0.4rem', padding: '0.75rem', background: 'rgba(255,255,255,0.03)', borderRadius: 'var(--r-sm)', border: '1px solid rgba(255,255,255,0.06)' }}>
      ...
      <motion.div className="glass-panel" style={{ padding: '2rem', display: 'flex', flexDirection: 'column', gap: '1.2rem' }} {...su(0.24)}>
        <h2 style={{ margin: 0, fontSize: '1.3rem', fontWeight: 800, fontFamily: "'Space Grotesk', 'Outfit', sans-serif" }}>
          {t('chi-sono.saluto')} 👋
        </h2>
        <p style={{ color: 'var(--text-main)', lineHeight: 1.9, fontSize: '1rem', margin: 0 }}>
          {t('chi-sono.bio')}
        </p>

        <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap', marginTop: '0.5rem' }}>
          <Link to="/twitch" className="btn btn-primary"><Twitch size={15} /> {t('chi-sono.seguimi')}</Link>
          <Link to="/gioco" className="btn btn-ghost"><Gamepad2 size={15} /> {t('chi-sono.scopri')}</Link>
        </div>
      </motion.div>

      <MandyMashwearSection lingua={lingua} />

      <motion.div className="glass-panel" style={{ padding: '1.75rem' }} {...inView(0)}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))', gap: '0.85rem' }}>
          {INTERESSI.map(i => (
            <motion.div key={i.label} className="glass-card" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.6rem', padding: '1rem 0.75rem', textAlign: 'center', borderColor: `${i.color}20`, '--card-glow': i.color }} whileHover={{ scale: 1.04, y: -3 }} whileTap={{ scale: 0.97 }} transition={{ type: 'spring', stiffness: 380, damping: 22 }}>
      ...
      <motion.div className="glass-panel" style={{ padding: '1.75rem', display: 'flex', flexDirection: 'column', gap: '1rem' }} {...inView(0.05)}>
        ...
        <Link to="/twitch" className="btn btn-primary" style={{ alignSelf: 'flex-start', background: 'linear-gradient(135deg,#9146FF,#6441a5)' }}>
